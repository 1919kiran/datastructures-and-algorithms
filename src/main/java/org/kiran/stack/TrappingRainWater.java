class Solution {
     ...
     * If the left max is smaller, then the amount of water we can trap on the left side is guaranteed, 
     * no matter what happens on the right. Why? Because even if the right max increases further, 
     * it doesn’t affect the result for left side — the bottleneck is the left max and vice versa
     */
    public int trappingRainWaterOptimal(int[] blocks) {
        int N = blocks.length;
        if (N < 3) return 0; // cannot trap water with less than 3 blocks
